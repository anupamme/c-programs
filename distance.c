...
model_data* processModel(char *file_name){
	...
	result->words = (long long *)malloc(sizeof(long long));
	result->size = (long long *)malloc(sizeof(long long));
	fscanf(f, "%lld", result->words);
 	fscanf(f, "%lld", result->size);
 	result->vec = (float *)malloc((*result->size) *  sizeof(float));
...
/*
	return: type is char* which points to array.
	(word, distance)
*/
char* convertToString(similar_word_iota* var){
//	int size = sizeof(var)/sizeof(var[0]);
	int size = N;
	int i;
	similar_word_iota sim;
	char * result = (char *)malloc(30*size);	// calculate this later.
	char *tmp;
	char tmpFloat[80];
	result[0] = '[';
	for (i = 0; i < size; i++){
		...
		strcat(tmp, sim.word);
		strcat(tmp, &"\",");
		sprintf(tmpFloat, "%f", sim.distance);
		strcat(tmp, tmpFloat);
/*		if (i == size - 1)
			strcat(tmp, &"]");
		else*/
...
similar_word_iota* findSimilarWords(model_data *modelData, char* word1){
    long long words = *modelData->words;
    int c;
    int a = *modelData->a;
    int b = *modelData->b;
    ...
    similar_word_iota *result_iota = (similar_word_iota *)malloc(N*sizeof(similar_word_iota));
    float bestd[N];
    char* bestw[N];
    float len = *modelData->len;
    float dist;
...
//    for (a = 0; a < N; a++) { result.bestd[a] = bestd[a]; result.bestw[a] = bestw[a];}
    for (a = 0; a < N; a++) { result_iota[a].distance = bestd[a]; result_iota[a].word = bestw[a];}
    return result_iota;
}
